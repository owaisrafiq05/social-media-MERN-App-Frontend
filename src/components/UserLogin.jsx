    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.status) {
                ...
                localStorage.setItem('userID', response.data.data._id); // Save userID in localStorage
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-green-800 to-teal-600">
            <form className="max-w-md w-full bg-white rounded-xl shadow-2xl p-8 space-y-8" onSubmit={handleSubmit}>
                <h2 className="text-center text-4xl font-extrabold text-gray-900">User Login</h2>
                {error && (
                ...
                <div className="text-center text-gray-600">
                    Not a user?{' '}
                    <Link to="/user/signup" className="text-teal-500 hover:underline">
                        Signup
                    </Link>
                </div>
                <button
